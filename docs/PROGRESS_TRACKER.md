...
Current phase: M5 stabilization and freeze prep (started)
Current step: M5.1 UI/gameplay parity regression sweep + backlog triage
  ...
  - [x] GP.3.2 Remaining world gameplay edge cases (decorator/populator + subsystem corner cases)
    ...
    - [x] GP.3.2b Decorator/populator + remaining subsystem corner cases
      ...
      - [x] Fluid rendering/interaction parity follow-up (`LiquidTile` visual + use semantics)
        ...
        - Scope extension: fluid/chunk render ordering path now tracks C++ layer semantics (`LiquidTile::getRenderLayer` + level render layer split), including water in translucent layer `1` while lava/redstone/cutout geometry remain in layer `0` to avoid non-parity transparent-pass hitch behavior.
    ...
    - [x] GP.3 parity debt backlog (closed)
      - [x] Replace temporary chunk mesh rebuild budget/queue throttles with a C++ parity-accurate render/update path (runtime now uses nearest-first mesh rebuild selection with C++-anchored very-near burst handling, without non-parity env-gated rebuild budgets).
      - [x] Revisit fluid render ordering/parity path against original client render layers to remove remaining chunk-water hitch edge cases without non-parity throttles.
      - [x] Normalize tracker/docs notes with runtime defaults after parity debt close (fluid pass wording + `CHUNK_LOAD_RADIUS` note alignment).
...
- [x] M4.4 Chunk terrain meshing/atlas rendering path
  ...
  - Integration tests: `tests/terrain_meshing_integration.rs`
  - Validation: `cargo test` and `cargo check --features bevy_client --bin bevy_client` passed (via VS toolchain environment).
- [x] M4.5 Gameplay UI parity completion (closed)
  ...
  - [x] M4.5.5 Creative inventory UI parity baseline
    ...
    - Empty-hand parity hotfix: first-person fist/arm now uses a skinned `HumanoidModel::arm0`-equivalent mesh path (instead of cube fallback), and empty-hand transform math now follows the C++ `ItemInHandRenderer` + `PlayerRenderer::renderHand` chain (`-1,3.6,3.5` then `120/200/-135` rotations and `+5.6` translation), fixing the "only visible while swinging" regression.
    - Empty-hand mesh wiring follow-up: removed an accidental runtime overwrite that was still forcing the fist entity mesh to block id `3` every frame, so empty-hand now consistently renders the arm mesh path instead of a flat/cube-looking fallback.
    - Redstone visual parity hotfix: redstone dust meshing now uses C++ colour-table-aligned unlit/lit tint values (`Tile_RedstoneDustUnlit=0x4c0000`, lit max `0xff3200`) instead of grayscale, and internal powered-wire atlas mapping no longer points at a transparent fallback tile.
    - Redstone/attachment placement parity follow-up: redstone wire geometry now follows C++ connection-state expansion/cut semantics (so adjacent dust links instead of isolated squares) with transparent-pass compositing to remove dark background quads; torches/levers now render as thin non-cube shapes from block-data orientation instead of full cubes; and placement now writes C++-style block metadata for torch/lever attachment faces plus piston facing (`getNewFacing` yaw/height rules) while piston meshing now reads facing data so sideways pistons render front/side textures on the correct faces.
    - Creative parity follow-up: refreshed creative tab source tables against `IUIScene_CreativeMenu::staticCtor` (transport/redstone additions including activator rail/daylight/dropper/hopper/comparator/trapped/weighted plates and TNT/Hopper minecarts; tools additions for empty-map, lead, horse armor, and full max-level enchanted-book set; misc additions for name tag/nether star, expanded spawn eggs including bat/witch/horse/donkey/mule, and firework preset entries), and extended item icon atlas mapping for newly surfaced item IDs.
    - Gameplay input parity follow-up: added mouse-wheel hotbar slot cycling in survival gameplay (wraparound behavior), matching expected legacy first-person control flow.
    - Interaction/combat parity follow-up: sword-selected left-click now no longer destroys targeted blocks (hit feedback only) while right-click applies first-person sword defend pose (`UseAnim_block` transform chain), block targeting now renders C++-style black selection outline in gameplay view (including thin-shape bounds for torches/levers), and creative inventory gained slot drag pickup/place semantics with hotbar-to-inventory disposal behavior for creative cleanup flow.
    - Creative tab label parity follow-up: Decorations tab title now matches `IUIScene_CreativeMenu::staticCtor` string (`"Decoration"` singular), with integration assertion coverage.
    - Creative catalog parity follow-up: restored remaining `IUIScene_CreativeMenu::staticCtor` base IDs in tab order for Structures/Decoration/Misc (coal block `173`, fence variants `85/113/101`, hay bale `170`, beacon `138`) and aligned Structures ordering around clay/fence segments to match C++ list flow.
    - Creative paging parity follow-up: selector paging now matches `TabSpec::populateMenu` scroll-window semantics (`m_staticPerPage = 10`), so page advances by one row (10 entries) across a 5x10 viewport instead of jumping full 50-slot pages; page-count math now uses row-window overlap parity.
    - Brewing tab parity follow-up: removed non-parity dynamic potion-group cycling and restored the active C++ static-group model (`Brewing`, `Potions_Level2_Extended`, `Potions_Extended`, `Potions_Level2`, `Potions_Basic`) with in-order concatenation + row-scroll paging behavior.
    - Files: `src/client/creative_ui.rs`, `src/client/mod.rs`, `src/client/clouds.rs`, `src/client/particles.rs`, `src/client/terrain_meshing.rs`, `src/bin/bevy_client.rs`, `src/client/asset_pipeline.rs`, `src/world/item_use.rs`, `src/world/simulation.rs`, `tests/client_creative_ui_integration.rs`, `tests/client_asset_pipeline_integration.rs`, `tests/client_clouds_integration.rs`, `tests/client_particles_integration.rs`, `tests/terrain_meshing_integration.rs`, `tests/player_fluid_physics_integration.rs`, `tests/crafting_item_use_integration.rs`, `tests/client_inventory_ui_integration.rs`, `tests/client_interaction_integration.rs`, `docs/PARITY_MATRIX.md`, `docs/PORT_PLAN_V2.md`, `../../../AGENTS.md`
    ...
    - Validation follow-up: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, `cargo test --test client_clouds_integration`, and `cargo test --test client_particles_integration` passed.
    - Validation follow-up: `cargo fmt` and `cargo check --features bevy_client --bin bevy_client` passed after empty-hand mesh wiring fix.
    - Validation follow-up: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, `cargo test --test terrain_meshing_integration`, and `cargo test --test redstone_lifecycle_integration` passed.
    - Validation follow-up: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, `cargo test --test terrain_meshing_integration`, `cargo test --test redstone_lifecycle_integration`, and `cargo test --test client_interaction_integration` passed (`cargo test --features bevy_client --tests` still blocked if `target/debug/bevy_client.exe` is running/locked by another process on Windows).
    - Validation follow-up: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, `cargo test --test client_creative_ui_integration`, `cargo test --test terrain_meshing_integration`, and `cargo test --test redstone_lifecycle_integration` passed.
    - Validation follow-up: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, `cargo test --test client_creative_ui_integration`, `cargo test --test client_interaction_integration`, `cargo test --test terrain_meshing_integration`, and `cargo test --test redstone_lifecycle_integration` passed.
    - Validation follow-up: `cargo check --features bevy_client --bin bevy_client` and `cargo test --test client_creative_ui_integration` passed after creative tab title parity update.
    - Validation follow-up: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, and `cargo test --test client_creative_ui_integration` passed after creative catalog ID/order parity updates.
    - Validation follow-up: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, and `cargo test --test client_creative_ui_integration` passed after creative row-scroll paging parity updates.
    - Validation follow-up: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, and `cargo test --test client_creative_ui_integration` passed after brewing static-group paging parity update.
    - Validation closure: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, `cargo test --test client_creative_ui_integration`, `cargo test --test client_inventory_ui_integration`, and `cargo test --test client_hotbar_ui_integration` passed; M4.5.5 baseline marked complete.
...
- 2026-03-04: M4.5.5 empty-hand visibility hotfix: replaced empty-hand cube fallback with a skinned right-arm mesh path (`HumanoidModel::arm0` UV/box shape intent) and switched first-person empty-hand transform to the C++ `ItemInHandRenderer`/`PlayerRenderer::renderHand` chain, restoring persistent fist visibility at rest (not only during swing).
- 2026-03-04: M4.5.5 empty-hand mesh wiring fix: removed stale per-frame mesh override (`ui_item_mesh_handle(3)`) from the fist render path so empty-hand view now stays on the intended arm mesh instead of reverting to block-cube geometry.
- 2026-03-04: M4.5.5 redstone dust tint hotfix: corrected redstone wire render tinting to C++ colour-table semantics (unlit/lit red) and removed transparent powered-wire atlas fallback mapping so placed dust no longer appears grayscale/black-white.
- 2026-03-04: M4.5.5 redstone/placement geometry follow-up: implemented C++-style redstone dust connection-state geometry expansion/cut behavior (with transparent compositing so dust no longer draws dark square backgrounds), added thin block-data-driven torch/lever meshes instead of full-cube fallbacks, wrote placement metadata for torch/lever attachment + piston facing, and switched piston face texturing to read facing data so sideways placement renders correctly.
...
- 2026-03-05: GP.3 mesh-update parity debt closure pass (partial): removed temporary streaming mesh rebuild env budgets (`LCE_DEFERRED_MESH_REBUILDS_PER_FRAME`, `LCE_MAX_MESH_REBUILDS_PER_FRAME`) and switched chunk-window mesh update selection to a C++-anchored nearest-first path with very-near burst cap (`GameRenderer.cpp` `MAX_DEFERRED_UPDATES` intent), while keeping immediate rebuilds for newly loaded/missing visible mesh chunks; validated with `cargo check --features bevy_client --bin bevy_client`, `cargo test --features bevy_client --test client_chunk_streaming_integration --test client_world_worker_integration --test terrain_meshing_integration`.
- 2026-03-05: GP.3 fluid render-order parity debt closure: switched terrain mesh layer classification to C++-anchored render-layer semantics (`LiquidTile::getRenderLayer` intent) so water routes through translucent layer `1` while lava/redstone/cutout meshes remain in layer `0`, updated terrain opaque material to alpha-mask cutout behavior for layer-0 parity, and synchronized docs/runtime notes (removed stale global-fluid-pass wording + outdated active-step references); validation: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, and `cargo test --features bevy_client --test terrain_meshing_integration --test client_chunk_streaming_integration --test client_world_worker_integration`.
- 2026-03-05: Runtime tuning follow-up by request: lowered default render distance to `CHUNK_LOAD_RADIUS=12` (from `16`) in `bevy_client` while keeping `LCE_CHUNK_LOAD_RADIUS` override behavior; validation: `cargo check --features bevy_client --bin bevy_client`.
- 2026-03-05: M4.5.5 creative catalog follow-up: restored missing C++ creative base entries and ordering in Structures/Decoration/Misc tabs (`173`, `85/113/101`, `170`, `138`) from `IUIScene_CreativeMenu::staticCtor`, with added integration assertions for coal/fence ordering and hay/beacon placement; validation: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, and `cargo test --test client_creative_ui_integration`.
- 2026-03-05: M4.5.5 creative paging follow-up: switched selector pagination to C++ row-scroll semantics from `TabSpec::populateMenu` (`startIndex = page * columns` with 5-row window overlap) and added integration coverage for one-row page stride/overlap behavior; validation: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, and `cargo test --test client_creative_ui_integration`.
- 2026-03-05: M4.5.5 brewing paging-model parity follow-up: removed non-parity dynamic-group cycling and aligned brewing tab data flow to the active C++ static-group path (`Brewing` + potion groups in order) with row-scroll pagination; validation: `cargo fmt`, `cargo check --features bevy_client --bin bevy_client`, and `cargo test --test client_creative_ui_integration`.
- 2026-03-05: M4.5 gameplay UI parity closure: completed M4.5.5 creative baseline parity sweep (title/catalog/paging/brewing model parity) and advanced active tracking to M5 stabilization + regression sweep.
