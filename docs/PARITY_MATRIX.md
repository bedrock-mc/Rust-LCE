...
| Terrain rendering | `TileRenderer`, `LevelRenderer`, `ParticleEngine::destroy`, `TerrainParticle` | `src/client/terrain_meshing.rs`, `src/client/particles.rs`, `src/bin/bevy_client.rs` | yellow | Atlas/shape parity improved; break particles use bottom-face terrain tile sub-UV sampling (`getTexture(0,data)` intent), redstone wire now applies lit/unlit tint + connection geometry expansion, torch/lever now use thin non-cube meshes, piston front-face texturing now follows block-data facing, and terrain mesh layer routing now follows C++ render-layer intent (water layer `1`, lava/redstone/cutouts layer `0`), but broader particle/shader/render-order parity is still pending.
| Gameplay UI | `Gui`, `InventoryScreen`, `CraftingScreen`, creative menu | `src/client/*_ui.rs`, `src/bin/bevy_client.rs` | yellow | M4.5.5 baseline is now closed. Menu labels prefer staged Mojangles TTF, creative tab/source lists now include late-tab parity additions (spawn eggs/horse armor/lead/map variants + transport/redstone late items), the Decorations tab title now matches C++ `"Decoration"` singular naming, missing Structures/Decoration/Misc base entries from `IUIScene_CreativeMenu::staticCtor` are restored (`173`, `85/113/101`, `170`, `138`), selector paging now follows C++ row-scroll window semantics (`startIndex = page * 10` over a 5x10 viewport), and brewing now follows the active C++ static-group model (no dynamic-group cycling); hotbar mouse-wheel selection is restored, sword right-click defend pose + hit-outline feedback are wired, and survival HUD now renders health + hunger icons plus XP bar/level from runtime player state.
...
4. Run M5 UI/gameplay regression hardening on recent parity closures (creative paging/catalog, clouds/water, held-item rendering).
5. Complete audio cue parity (XSB cue table + deterministic bank event routing).
6. Keep networking deferred until offline parity lock, then bootstrap protocol/session migration.
